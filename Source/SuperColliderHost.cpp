...
juce::String SuperColliderHost::makeBridgeScript() const
    {
        ...
        return "(\n"
               ...
               "    var low = Lag.kr(wfLow.clip(20, 19900), 0.012);\n"
               "    var high = Lag.kr(wfHigh.clip(low + 5, 20000), 0.012);\n"
               ...
               "        low = low.clip(20, 19900);\n"
               "        high = high.clip(low + 5, 20000);\n"
               "        edgeLag = 0.012;\n"
               "        low = Lag.kr(\\wfLow.kr(low), edgeLag).clip(20, 19900);\n"
               "        high = Lag.kr(\\wfHigh.kr(high), edgeLag).clip(low + 5, 20000);\n"
               "        3.do {\n"
               "            sig = BHiPass4.ar(sig, low, 0.56);\n"
               "            sig = BLowPass4.ar(sig, high, 0.56);\n"
               "        };\n"
